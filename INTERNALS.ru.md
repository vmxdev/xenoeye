...
Иногда разработчики коллекторов не очень заморачиваются с производительностью и экономией ресурсов. Всегда можно увеличить частоту семплирования и тем самым уменьшить количество экспортируемых фловов в секунду.

Но нужно принимать во внимание, что например при частоте семплирования 1:1000 вы не видите буквально 99.9% трафика. Чем выше частота, тем меньше точность отчетов и тем хуже видны аномалии в трафике.
...
После того, как коллектор прочитал конфигурационный файл `mo.conf`, он пытается скомпилировать фильтр в байткод. Если получается, байткод сохраняется в памяти.
...
В коллекторе есть два типа netflow-полей. Первый тип — netflow-поля `IN_BYTES` и `IN_PKTS`. Эти поля "зашиты" в код как агрегируемые. Если они встречаются в массиве полей для экспорта, то данные из этих полей будут суммироваться.
...
Возможно, позже мы переделаем архитектуру и можно будет добавить все поля без потери производительности обработки.
...
В каждом флове есть время когда роутер начал и когда закончил наблюдение. Коллектор **не смотрит** на это время. Временем флова считается текущее время на сервере. Это немного уменьшает точность, но сильно упрощает обработку, особенно обработку скользящих средних.
...
В коллекторе используются [https://ru.wikipedia.org/wiki/%D0%A1%D0%BA%D0%BE%D0%BB%D1%8C%D0%B7%D1%8F%D1%89%D0%B0%D1%8F_%D1%81%D1%80%D0%B5%D0%B4%D0%BD%D1%8F%D1%8F#%D0%9A%D1%83%D0%BC%D1%83%D0%BB%D1%8F%D1%82%D0%B8%D0%B2%D0%BD%D0%BE%D0%B5_%D1%81%D0%BA%D0%BE%D0%BB%D1%8C%D0%B7%D1%8F%D1%89%D0%B5%D0%B5_%D1%81%D1%80%D0%B5%D0%B4%D0%BD%D0%B5%D0%B5](куммулятивные скользящие средние)

Значение байт или пакетов в окне вычисляется по формуле:
...
V — значение величины в новом пришедшем флове
