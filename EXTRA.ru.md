...
Лучше обновлять GeoIP базы вручную. Алгоритм обновления приблизительно такой же, как и для первого использования.
...
Для классификации в коллекторе есть такие вспомогательные функции:
  ...
  * `divl(aggr1, aggr2, N)` - деление c округлением вниз к целой степени
...
`divl(octets,packets,N)` - деление с окpуглением вниз к целой степени. Если нужно грубо классифицировать по размеру пакетов, например как "маленькие", "средние", "большие" - можно использовать эту функцию.
