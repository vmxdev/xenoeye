# Пошаговая инструкция по установке и настойке коллектора
  ...
  * [Можно ли использовать GeoIP?](#можно-ли-использовать-geoip)
...
``` sql
=> select sum(octets) from engress_all where time >= now() - interval '1 hour';
    sum
-------------
 14600823461
(1 row)
...
Самая большая сложность для open source проектов в работе c GeoIP - это отсутствие свободно распространяемых GeoIP баз.

Если вы знаете проект, который распространяет базы без дополнительных условий, без требования изменить лицензию - напишите в issue.

В коллекторе нет поддержки GeoIP, но если у вас есть какая-то база (например, MaxMind), то вы можете сделать географический объект мониторинга с помощью IP-списков.

Cоздайте файл `/var/lib/xenoeye/iplists/REGION` и поместите туда все IP-адреса нужного региона. После этого можно будет создать объекты мониторинга, в которых будет только входящий/исходящий трафик региона.
...
Для многих применений этого достаточно.

Но если вам нужно точно и быстро реагировать на увеличение скорости трафика, то с таким подходом могут возникнуть сложности:
...
Для того, чтобы избежать этого, можно использовать другой подход - подсчет средней скорости трафика в скользящих окнах.
...
Для работы скользящих окон коллектору не нужна СУБД, конфигурация берется из файлов, все рассчеты производятся внутри.

`/var/lib/xenoeye/mo/http_spam/mo.conf`
```
	"mavg": [
		{
			"name": "mavg1",
			...
			"overlimit": [
				{
					...
					"back2norm-time": 60,
					"default": [10000000],
					"limits": "/var/lib/xenoeye/mo/http_spam/limits1.csv",
...
``` sh
$ touch /var/lib/xenoeye/mo/http_spam/mavg1.d
```
После этого коллектор начнет периодически сбрасывать значения скользящих средних в файл `/var/lib/xenoeye/mo/http_spam/mavg1.dump`.
...
``` sh
$ touch /var/lib/xenoeye/mo/http_spam/mavg1.a
```

После этого каждый дамп будет добавляться к файлу `/var/lib/xenoeye/mo/http_spam/mavg1.adump`

Не забудьте потом удалить файл `.a`, даже на небольших сетях файл может получиться огромного размера
...
Скрипты запускаются с такими параметрами:
  ...
  3. Полное имя файла с уведомлениями
  4. Декодированные netflow-поля, в том порядке как записано в `"fields"`.
  5. Значение поля, которое превысило порог
  6. Значение порога

С такой конфигурацией в файле `/var/lib/xenoeye/mo/http_spam/mo.conf`:
```
{
	"name": "mavg1",
	"fields": ["src host", "proto", "octets"],
...
`/var/lib/xenoeye/scripts/on-start.sh http_spam mavg1 level1 /var/lib/xenoeye/notifications/http_spam-mavg1-level1-15.22.13.99-6 15.22.13.99 6 1234567 1000000`
...
С помощью скриптов, которые запускаются при превышении порога и при возвращении трафика в норму, можно оповещать пользователя используя мессенджеры. Мы пользуемся Telegram, скрипты для запуска робота и уведомлений находятся в каталоге ...
