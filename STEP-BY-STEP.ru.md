...
```
Будет создана таблица `ingress_by_dst` с полями `time`, `packets`, `octets`, `dst_host`. Каждые 30 секунд в таблицу будет добавляться время, все IP-адреса, на которые шел трафик за эти 30 секунд, количество пакетов и байт, пришедших на каждый IP адрес.
...
```
В таблицу `ingress_octets_by_src` добавляются IP адреса источников и количество байт, которые пришли от них. Так как адресов источников может быть много, мы ограничиваем их количество. За 30 секунд будет экспортироваться топ-5 источников по количеству байт. `"limit": 5` указывает системе количество записей. Спецификатор `desc` в названии поля (`"packets desc"`) говорит о том, что нужно сортировать по количеству пакетов от большего количества к меньшему.
...
Для исходящего трафика создадим конфиг-файл с таким набором экспортируемых данных:
...
После перезапуска коллектора он начнет генерировать файлы с данными.
