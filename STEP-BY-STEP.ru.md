...
Иногда, особенно в крупных сетях, используется несколько `*flow`-коллекторов и оборудование не позволяет экспотрировать `*flow` еще на одно устройство. В этом случае можно подать на коллектор зеркало/SPAN `*flow`-трафика, или продублировать с помощью samplicator-утилит, он сможет получать его с интерфейса используя захват pcap.
...
Можете запустить коллектор на нескольких UDP-портах и подавать на каждый порт Neflow разных роутеров.

Если вы захватываете `*flow`-трафик с помощью pcap, можно так же распределить нагрузку, указывая в секции разные роутеры:
...
Этот способ распределения нагрузки может покаться слишком "ручным", многие коллекторы пытаются нагрузить CPU автоматически и равномерно. Однако, при резком росте трафика (например, при DoS/DDoS-атаках) системы с равномерным распределением могут захлебнуться **целиком** и искажать значения по всем роутерам. В то время как при такой схеме сильно будут нагружены только некоторые CPU (обрабатывающие трафик роутеров под атакой), остальные роутеры это не затронет.
