# Пошаговая инструкция по установке и настойке коллектора
  ...
  * [IP-списки](#ip-списки)
  * [Настраиваем данные для экспорта в СУБД]()
  * [Экспорт в СУБД]()
...
Перезапустите коллектор и посмотрите что он выводит на экран. Он должен показывать только те фловы, у которых `IPv4 dst addr` находится в сети `10.11.12.0/24`.
...
Для IPv6 нужно указывать поля с префиксом `6`, например "`src net6 bogon and dst net6 bogon`"
...
Если коллектор показывает правильные фловы для объекта мониторинга, можно убирать печать фловов и добавлять секции экспорта в СУБД.
...
```
{
	...
	"fwm": [
		{
			/* общее количество байт на нашу сеть, по всем сетям */
			"name": "all",
			"fields": ["octets"]
		},
		{
			/* dst IP и количество байт по каждому адресу */
			"name": "octets_by_dst",
			"fields": ["octets", "dst host"]
		},
		{
			/* номера протоколов и количество байт по каждому протоколу */
		...
		{
			/* IP адрес, порт назначения и количество байт для каждого уникального dst IP + dst порт */
			"name": "octets_by_dst_and_port",
			"fields": ["octets desc", "dst host", "dst port"],
			"limit": 5,
...
Перезапустите коллектор, и он начнет собирать и агрегировать данные для экспорта.

Каждая секция в "fwm" соответствует таблице в СУБД. Название таблицы получается из названия объекта мониторинга + `_` + название секции.

В каждой секции должно быть название и список netflow-полей, которые будут использоваться. Кроме этого можно изменять время, за которое агрегируются данные ("time", по умолчанию 30 секунд) и ограничивать размер экспортируемых за этот период времени строк.
...
```
Секция с именем "all" инструктирует систему создать таблицу с именем `ingress_all`. У таблицы будет 2 поля: `time` и `octets`. Каждые 30 секунд в таблицу будет добавляться запись со временем и с объемом трафика в байтах по всему этому объекту мониторинга.
...
```
Cистема создаст таблицу `ingress_octets_by_dst` с полями `time`, `octets`, `dst_host`. Каждые 30 секунд в таблицу будет добавляться время, все IP-адреса, на которые шел трафик за эти 30 секунд и количество байт, пришедших на каждый IP адрес.
...
```
В таблицу `ingress_octets_by_proto` будет добавляться номер протокола и количество входящих байт по каждому протоколу
...
```
В таблицу `ingress_packets_by_src` добавляются IP адреса источников и количество пакетов, которые пришли от них. Так как адресов источников может быть много, мы ограничиваем их количество. За 30 секунд будет экспортироваться top-5 источников по количеству пакетов. `"limit": 5` указывает системе количество записей. Спецификатор `desc` в названии поля (`"packets desc"`) говорит о том, что нужно сортировать по количеству пакетов в направлении от большего количества к меньшему.

Данные будут экспортироваться каждые 30 секунд приблизительно в таком формате:
```
time                   packets      src_host
...
```
Последняя строка с NULL вместо IP адреса - это сумма трафика (пакетов) всех остальных хостов, которые не вошли в top-5.
...
В таблицу кроме IP адреса назначения добавляется еще и порт назначения. Записи перед экспортом будут отсортированы по убыванию байт, будут выбраны только top-5. Кроме этого, файл экспорта будет генерироваться не каждые 30, а каждые 10 секунд.

Файл экспорта будет содержать данные в таком формате:
```
time                   octets       dst_host   dst_port
2023-01-09 22:14:33    3967103145   1.2.3.4    443
2023-01-09 22:14:33    3725964504   1.2.3.4    80
2023-01-09 22:14:33    2189341567   4.3.2.1    33015
2023-01-09 22:14:33    2152099043   2.3.4.1    0
2023-01-09 22:14:33    1475885501   3.2.1.4    1194
2023-01-09 22:14:33   50575004350   NULL
```
...
После всех манипуляций, описанных выше, у вас начнут заполняться таблицы СУБД. Значит можно начать делать отчеты.
