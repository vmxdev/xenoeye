...
Каждая подсекция в "fwm" соответствует таблице в СУБД.

Например, секция с именем "all" инструктирует систему создать таблицу с именем `ingress_all`. У таблицы будет 2 поля: `time` и `octets`. Каждые 30 секунд в таблицу будет добавляться запись со временем и с количеством трафика (байт) по всему этому объекту мониторинга.

Следующая секция немного сложнее. Будет создана таблица `ingress_octets_by_dst` с полями `time`, `octets`, `dst_host`. Каждые 30 секунд в таблицу будет добавляться время, все IP-адреса, на которые шел трафик за эти 30 секунд и количество байт, пришедших на каждый IP адрес.
