...
```
{
	...
	"fwm": [
		{
			/* общее количество байт на нашу сеть, по всему объекту мониторинга */
			"name": "all",
			"fields": ["octets"]
		},
		{
...
Конфиг приведен в качестве примера, скорее всего вам нужно будет мониторить другие наборы данных.

В каждом элементе должно быть название (`"name"`) и список netflow-полей, которые будут использоваться (`"fields"`). Кроме этого можно изменять время, за которое агрегируются данные (`"time"`, по умолчанию 30 секунд) и ограничивать размер экспортируемых за этот период времени строк (по умолчанию экспортируются все данные). Подробное описание файлов `mo.conf` находится ...
...
```
Элемент с именем "all" инструктирует систему создать таблицу `ingress_all`. У таблицы будет 2 поля: `time` и `octets`. Каждые 30 секунд в таблицу будет добавляться запись со временем и объемом трафика в байтах по всему объекту мониторинга.
...
```
В таблицу `ingress_packets_by_src` добавляются IP адреса источников и количество пакетов, которые пришли от них. Так как адресов источников может быть много, мы ограничиваем их количество. За 30 секунд будет экспортироваться топ-5 источников по количеству пакетов. `"limit": 5` указывает системе количество записей. Спецификатор `desc` в названии поля (`"packets desc"`) говорит о том, что нужно сортировать по количеству пакетов в направлении от большего количества к меньшему.

Данные будут экспортироваться каждые 30 секунд в таком формате:
```
...
```
Последняя строка с NULL вместо IP адреса - это сумма трафика (пакетов) всех остальных хостов, которые не вошли в топ-5.


Для исходящего трафика создадим конфиг-файл с другим набором экспортируемых данных:

`/var/lib/xenoeye/mo/egress/mo.conf`:
```
{
	"filter": "src net my-net",
	...
	"fwm": [
		{
			/* общее количество байт из нашей сети, по всему объекту мониторинга */
			"name": "all",
			"fields": ["octets"]
		},
		{
			/* src IP и количество байт по каждому адресу */
			"name": "octets_by_src",
			"fields": ["octets", "src host"]
		},
		{
			/* dst IP адрес и количество байт с каждого адреса */
			"name": "octets_by_dst",
...
В каталоге `scripts/` находится файл `fill-db.sh`, который это делает. Измените в нем параметры подключения (`user:password@127.0.0.1:5432/database`) и опционально пути к каталогам. База данных должна существовать, пользователь должен иметь права на запись.
