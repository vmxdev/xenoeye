...
Но если вам нужно точно и быстро реагировать на всплески трафика, то с таким подходом могут возникнуть сложности:

  * Резкие всплески могут "раствориться" в достаточно длинном окне, их будет не видно
  * Система сможет реагировать на всплески только после того как закончится временно́е окно и экспортируются данные
...
Можно использовать два источника для вычисления порогов. Брать данные из СУБД или попросить систему показывать текущие значения скользящих средних.
...
`mavg1` - название скользящего среднего. После этого коллектор начнет периодически сбрасывать значения скользящих средних в файл `/var/lib/xenoeye/mo/egress/mavg1.dump`.
...
Если хотите посмотреть как изменяются скользящие средние в динамике, то можно создать файл
``` sh
