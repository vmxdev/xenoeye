			...
			"overlimit": [
				{
					...
					"back2norm-time": 60,
					"default": [10000000],
					"limits": "/var/lib/xenoeye/mo/egress/limits1.csv",
...
``` sh
$ touch /var/lib/xenoeye/mo/egress/mavg1.d
```
После этого коллектор начнет периодически сбрасывать значения скользящих средних в файл `/var/lib/xenoeye/mo/egress/mavg1.dump`.
...
``` sh
$ touch /var/lib/xenoeye/mo/egress/mavg1.a
```

После этого каждый дамп будет добавляться к файлу `/var/lib/xenoeye/mo/egress/mavg1.adump`
...
Скрипты запускаются с такими параметрами:
  ...
  4. Полное имя файла с уведомлениями
  5. Декодированные netflow-поля, в том порядке как записано в `"fields"`.
  6. Значение поля, которое превысило порог
  7. Значение порога

С такой конфигурацией в файле `/var/lib/xenoeye/mo/egress/mo.conf`:
```
{
	"name": "mavg1",
	"fields": ["src host", "proto", "octets"],
...
`/var/lib/xenoeye/scripts/on-start.sh egress mavg1 level1 /var/lib/xenoeye/notifications/egress-mavg1-level1-15.22.13.99-6 15.22.13.99 6 1234567 1000000`
...
При превышении порога кроме запуска скрипта можно включить "расширенную статистику". Это данные из окон фиксированного размера. Они помечаются параметром `"disabled" : true` и при старте неактивны. Как только превышается порог, эти элементы становятся активными. При возврате трафика в норму они опять становятся неактивными
